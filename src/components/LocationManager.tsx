...
const LocationManager: React.FC<LocationManagerProps> = ({ isOpen, onClose }) => {
  const [locations, setLocations] = useState<Location[]>([]);
  const [loading, setLoading] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [newLocation, setNewLocation] = useState({ name: '', display_name: '' });
  ...
  const handleAdd = async () => {
    if (!newLocation.name.trim() || !newLocation.display_name.trim()) {
    ...
    try {
      const created = await locationService.create({
        name: newLocation.name.toLowerCase().replace(/\s+/g, '_'),
        display_name: newLocation.display_name,
        is_active: true
      });
      setLocations([...locations, created]);
      setNewLocation({ name: '', display_name: '' });
  ...
  const handleEdit = (location: Location) => {
    ...
    setEditValues({ name: location.name, display_name: location.display_name });
  };
  ...
  const handleSaveEdit = async (id: string) => {
    ...
    try {
      const updated = await locationService.update(id, {
      ...
      <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-4 md:p-6 border-b">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-gray-800">Manage Locations</h2>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-4">
              ...
              {showAddForm && (
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 space-y-3">
                  <h4 className="font-semibold text-gray-700">Add New Location</h4>
                    ...
                    {editingId === location.id ? (
                      ...
                      <>
                        <div className="flex-1">
                          <p className="font-medium text-gray-800">{location.display_name}</p>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => handleDelete(location.id, location.display_name)}
