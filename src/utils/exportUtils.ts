...
export const exportSalaryPDF = (
  ...
  if (partTimeSalaries.length > 0) {
    ...
    // Calculate and display currency note breakdown (individual method)
    const noteBreakdown = calculateIndividualCurrencyNotes(partTimeSalaries);
    doc.setFontSize(10);
    doc.text('Currency Note Breakdown:', 20, currentY);
    currentY += 10;
    
    Object.entries(noteBreakdown).forEach(([denomination, count]) => {
      doc.text(`₹${denomination} = ${count}`, 30, currentY);
      currentY += 8;
    });
  }
...
export const exportPartTimeStaffToExcel = (
  allPartTimeAttendance: Attendance[],
  startDate: Date,
  endDate: Date
) => {
  // Filter attendance records by date range
  const filteredAttendance = allPartTimeAttendance.filter(record => {
    const recordDate = new Date(record.date);
    return recordDate >= startDate && recordDate <= endDate;
  });

  // Group by staff name and location
  const groupedData = filteredAttendance.reduce((acc, record) => {
    ...
    acc[key].totalDays += record.attendanceValue;
    acc[key].totalEarnings += record.salary || 0;
    acc[key].shifts.push({
      date: record.date,
      shift: record.shift,
      status: record.status,
      earnings: record.salary || 0
    });
    return acc;
  }, {} as Record<string, any>);

  // Convert to array and prepare Excel data
  const data = Object.values(groupedData).map((staff: any, index) => ({
    'S.No': index + 1,
    'Name': staff.name,
    'Location': staff.location,
    'Total Days': staff.totalDays,
    'Total Earnings': staff.totalEarnings
  }));

  // Group by location for subtotals
  const locationTotals = data.reduce((acc, row) => {
    const location = row.Location;
    if (!acc[location]) {
      acc[location] = { totalDays: 0, totalEarnings: 0, count: 0 };
    }
    acc[location].totalDays += row['Total Days'];
    acc[location].totalEarnings += row['Total Earnings'];
  ...
  Object.entries(locationTotals).forEach(([location, totals]) => {
    finalData.push({
      'S.No': '',
      'Name': `${location} Subtotal`,
      'Location': '',
      'Total Days': totals.totalDays,
      'Total Earnings': totals.totalEarnings
    });
  });

  // Add grand total
  const grandTotal = Object.values(locationTotals).reduce((sum: any, location: any) => ({
    totalDays: sum.totalDays + location.totalDays,
    totalEarnings: sum.totalEarnings + location.totalEarnings
  ...
  finalData.push({
    ...
    'Name': 'GRAND TOTAL',
    'Location': '',
    'Total Days': grandTotal.totalDays,
    'Total Earnings': grandTotal.totalEarnings
  });
  ...
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Part-Time Staff');
  ...
  XLSX.writeFile(wb, `part-time-staff-${startDateStr}-to-${endDateStr}.xlsx`);
};